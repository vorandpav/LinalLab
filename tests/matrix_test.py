...
class TestSparseMatrix(unittest.TestCase):
    ...
    def test_matrices_mul(self):
        ...
        for test in range(len(matrices_mul) - 1):
            m1 = matrix.Matrix(matrices_mul[test])
            m2 = matrix.Matrix(matrices_mul[test + 1])
            self.assertEqual(matrix.Matrix(matrices_mul_result[test]).get_list(), (m1 * m2).get_list())
